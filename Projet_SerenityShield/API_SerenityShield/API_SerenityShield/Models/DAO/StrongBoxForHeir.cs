...
namespace API_SerenityShield.Models.DAO
{
    public class IStrongboxForHeirs
    {
              ...
              bool prepayedInHeritence)
        {
            ...
            try
            {
                ...
                commDB = new OleDbCommand("INSERT INTO[dbo].[STRONGBOXFORHEIR]"
                          ...
                          + " ,[SCPK],[CODEID],[PREPAYEDINHERENCE],[DATEADDED],[UNLOCKED],[PAID])"
                        ...
                        + " '" + solSereNftPK + "','" + solUsrNftPK + "','" + solHeirNftPKs + "','" + scpk + "','" + codeId + "','" + prepayedInHeritence + "','" + dateNow + "',0,0)", connexDB);
        ...
        public string UpdateStrongboxForHeir(string idPolicy, string idstrongboxheir, string label, string listIdHeir,
            string Message, bool display, bool prepayedInHeritence, string walletPublicKey, string userSecretPK, string userPayingPK, string nftSere, string nftUser
            , string nftHeir, string solanaPK, string scPK, string codeID)
        {
            OleDbConnection connexDB;
            Connexion connect = new Connexion();
            ...
            try
            {
                ...
                commDB = new OleDbCommand("UPDATE [dbo].[STRONGBOXFORHEIR]"
                                          + " SET[ID_POLICY] ='" + idPolicy + "'"
                                            ...
                                            + "  ,[USER_SECRETPK] = '" + userSecretPK + "'"
                                            + "  ,[USER_PAYINGPK] = '" + userPayingPK + "'"
                                            + "  ,[NFT_SHARD_SERENITY] = '" + nftSere + "'"
                                            + "  ,[NFT_SHARD_USER] = '" + nftUser + "'"
                                            + "  ,[NFT_SHARD_HEIR] ='" + nftHeir + "'"
                                            + " ,[USER_SOLANAPK] = '" + solanaPK + "'"
                                            + " ,[SCPK] = '" + scPK + "'"
                                            + " ,[CODEID] = '" + codeID + "'"
        ...
        public string UnlockStrongboxForHeir(string idstrongbox)
        {
            OleDbConnection connexDB;
            Connexion connect = new Connexion();
        ...
        public string LockStrongboxForHeir(string idstrongbox)
        {
            OleDbConnection connexDB;
            Connexion connect = new Connexion();
        ...
        public string PaidStrongboxForHeir(string idstrongbox)
        {
            OleDbConnection connexDB;
            Connexion connect = new Connexion();
        ...
        public string UnPaidStrongboxForHeir(string idstrongbox)
        {
            OleDbConnection connexDB;
            Connexion connect = new Connexion();
        ...
        public IStrongboxForHeirs GetStrongboxForHeirById(string idSbHeir)
        {
            IStrongboxForHeirs u;
            OleDbConnection connexDB;
            Connexion connect = new Connexion();
            ...
            DataSet ds;
            commDB = new OleDbCommand("SELECT * FROM [dbo].[STRONGBOXFORHEIR] WHERE [ID_STRONGBOXFORHEIR] = '" + idSbHeir + "' AND ACTIVE=1", connexDB);

            adaptDB = new OleDbDataAdapter(commDB);
            ds = new DataSet();
            adaptDB.Fill(ds, "SB");

            u = new IStrongboxForHeirs();

            if (ds.Tables["SB"].Rows.Count > 0)
            {
                Cryptage cr = new Cryptage();
                string idstrongox = ds.Tables["SB"].Rows[0]["ID_STRONGBOXFORHEIR"].ToString();
                u.id = cr.EncryptHexa(ds.Tables["SB"].Rows[0]["ID_STRONGBOXFORHEIR"].ToString());
                ...
                u.unlocked = (bool)ds.Tables["SB"].Rows[0]["UNLOCKED"];
                u.paid = (bool)ds.Tables["SB"].Rows[0]["PAID"];
                ...
                u.messageForHeirs = ds.Tables["SB"].Rows[0]["MESSAGE_FOR_HEIRS"].ToString();
                string listeIdHeir = ds.Tables["SB"].Rows[0]["LIST_OF_HEIRS"].ToString();
                ...
                u.heirs = lh;
                u.displayToHeirs = (bool)ds.Tables["SB"].Rows[0]["DISPLAY"];
                string idperiod = ds.Tables["SB"].Rows[0]["ID_POLICY"].ToString();
                InactivityPeriod ip = new InactivityPeriod();
                ip = ip.GetInactivityPeriodById(idperiod);
                u.inactivityPeriod = ip.period;
                u.prepayedInHeritence = (bool)ds.Tables["SB"].Rows[0]["PREPAYEDINHERENCE"];
            }

            return u;
        }

        public IStrongboxForHeirs GetStrongboxForHeirByIdStronboxForHeir(string idStrongboxForHeir)
        {
            IStrongboxForHeirs u;
            OleDbConnection connexDB;
            Connexion connect = new Connexion();
            ...
            OleDbDataAdapter adaptDB;
            DataSet ds;
            commDB = new OleDbCommand("SELECT * FROM [dbo].[STRONGBOXFORHEIR] WHERE [ID_STRONGBOXFORHEIR] = '" + idStrongboxForHeir + "' AND ACTIVE=1", connexDB);
        ...
        public List<IStrongboxForHeirs> GetListStrongboxForHeirByIdUser(string idUser)
        {
            ...
            commDB = new OleDbCommand("SELECT     STRONGBOXFORHEIR.* "
                         + " FROM            STRONGBOXFORHEIR INNER JOIN"
                         + " CUSTOMER ON STRONGBOXFORHEIR.ID_CUSTOMER = CUSTOMER.ID_CUSTOMER"

                         + " INNER JOIN _USER ON CUSTOMER.ID_USER = _USER.ID_USER"

                         + " where _USER.ID_USER = '" + idUser + "' AND STRONGBOXFORHEIR.ACTIVE=1  order by LABEL ASC", connexDB);
            ...
            if (ds.Tables["SB"].Rows.Count > 0)
            {
                ...
                for (int i = 0; i < ds.Tables["SB"].Rows.Count; i++)
                {
                    ...
                    u.id = cr.EncryptHexa(ds.Tables["SB"].Rows[i]["ID_STRONGBOXFORHEIR"].ToString());
                    ...
                    u.unlocked = (bool)ds.Tables["SB"].Rows[i]["UNLOCKED"];
                    u.paid = (bool)ds.Tables["SB"].Rows[i]["PAID"];
                    content cont = new content();
                    IStrongboxContentWalletDex dex = new IStrongboxContentWalletDex();
                    cont.walletDex = dex.GetListStrongboxContentWalletDexByIdSbpersonnal(false, idstrongox);
                    IStrongboxContentWalletCex cex = new IStrongboxContentWalletCex();
        ...
        public bool IfExistStrongboxForHeirByIdUser(string idUser)
        {
            bool exist = false;
            ...
            connexDB.Open();
            OleDbCommand commDB;
            commDB = new OleDbCommand("if exists ( SELECT     STRONGBOXFORHEIR.* "
                         + " FROM            STRONGBOXFORHEIR INNER JOIN"
                         + " CUSTOMER ON STRONGBOXFORHEIR.ID_CUSTOMER = CUSTOMER.ID_CUSTOMER"

                         + " INNER JOIN _USER ON CUSTOMER.ID_USER = _USER.ID_USER"

                         + " where _USER.ID_USER = '" + idUser + "' AND STRONGBOXFORHEIR.ACTIVE=1 ) "
                            + " select 'True'  "
                            + " else "
    ...
    public class InactivityPeriod
    {
        ...
        public InactivityPeriod GetInactivityPeriodById(string idtInactivityPeriod)
        {
            ...
            {
                Cryptage cr = new Cryptage();
                u.id = cr.EncryptHexa(ds.Tables["InactivityPeriod"].Rows[0]["ID_POLICY"].ToString());
        ...
        public List<InactivityPeriod> GetListInactivityPeriod()
        {
            ...
            if (ds.Tables["InactivityPeriod"].Rows.Count > 0)
            {
                ...
                for (int i = 0; i < ds.Tables["InactivityPeriod"].Rows.Count; i++)
                {
                    ...
                    u.id = cr.EncryptHexa(ds.Tables["InactivityPeriod"].Rows[i]["ID_POLICY"].ToString());
    ...
    public class IStrongboxHeritage : IStrongboxForHeirs
    {
        public Owner owner { get; set; }

        public List<IStrongboxHeritage> GetListStrongboxForHeirByIdHeir(string cryptedidHeir)
        {
            List<IStrongboxHeritage> lu = new List<IStrongboxHeritage>();
            OleDbConnection connexDB;
            Connexion connect = new Connexion();
            ...
            OleDbDataAdapter adaptDB;
            DataSet ds;
            commDB = new OleDbCommand("SELECT  distinct STRONGBOXFORHEIR.* "
                                      + "  FROM            STRONGBOXFORHEIR"
                                      + "  where  LIST_OF_HEIRS like '%" + cryptedidHeir + "%' and DISPLAY = 1 order by LABEL ASC", connexDB);
            ...
            if (ds.Tables["SB"].Rows.Count > 0)
            {
                ...
                for (int i = 0; i < ds.Tables["SB"].Rows.Count; i++)
                {
                    IStrongboxHeritage u = new IStrongboxHeritage();

                    string idstrongox = ds.Tables["SB"].Rows[i]["ID_STRONGBOXFORHEIR"].ToString();
                    u.id = cr.EncryptHexa(ds.Tables["SB"].Rows[i]["ID_STRONGBOXFORHEIR"].ToString());
                    string idcustomer = ds.Tables["SB"].Rows[i]["ID_CUSTOMER"].ToString();
                    u.idOwner = cr.EncryptHexa(ds.Tables["SB"].Rows[i]["ID_CUSTOMER"].ToString());
                    u.label = ds.Tables["SB"].Rows[i]["LABEL"].ToString();
                    u.walletPublicKeyOwner = ds.Tables["SB"].Rows[i]["WALLETPUBLICKEY"].ToString();
                    u.secretPK = ds.Tables["SB"].Rows[i]["USER_SECRETPK"].ToString();
        ...
        public IStrongboxHeritage GetStrongboxForHeirByIdHeirAndStrongboxId(string cryptedidHeir, string StrongBoxId)
        {
            ...
            commDB = new OleDbCommand("SELECT  distinct STRONGBOXFORHEIR.* "
                                      + "  FROM            STRONGBOXFORHEIR"
                                      + "  where  LIST_OF_HEIRS like '%" + cryptedidHeir + "%' AND ID_STRONGBOXFORHEIR='" + cr.DecryptHexa(StrongBoxId) + "'  and DISPLAY = 1 order by LABEL ASC", connexDB);
            ...
            if (ds.Tables["SB"].Rows.Count > 0)
            {
                ...
                string idstrongox = ds.Tables["SB"].Rows[0]["ID_STRONGBOXFORHEIR"].ToString();
                u.id = cr.EncryptHexa(ds.Tables["SB"].Rows[0]["ID_STRONGBOXFORHEIR"].ToString());
                string idcustomer = ds.Tables["SB"].Rows[0]["ID_CUSTOMER"].ToString();
                u.idOwner = cr.EncryptHexa(ds.Tables["SB"].Rows[0]["ID_CUSTOMER"].ToString());
                u.label = ds.Tables["SB"].Rows[0]["LABEL"].ToString();
                u.walletPublicKeyOwner = ds.Tables["SB"].Rows[0]["WALLETPUBLICKEY"].ToString();
                u.secretPK = ds.Tables["SB"].Rows[0]["USER_SECRETPK"].ToString();
