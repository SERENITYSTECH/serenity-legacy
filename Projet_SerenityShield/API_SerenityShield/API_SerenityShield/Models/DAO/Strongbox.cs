...
namespace API_SerenityShield.Models.DAO
{
    ...
    public class IStrongboxPersonnal
    {
        ...
        public string UpdateLabelStrongboxPersonnal(string idstrongbox, string label,string walletPublicKeyOwner,
            string scPK,string codeID,string payingPK,string secretPK,string solanaPK,string solSereNftPK, string solUsrNftPK,string solHeirNftPKs)
        {
            OleDbConnection connexDB;
            Connexion connect = new Connexion();
            ...
            try
            {
                ...
                commDB = new OleDbCommand("UPDATE [dbo].[STRONGBOXPERSONNAL]"
                    + "  SET [LABEL] = '" + label + "' "         
                    + " ,[WALLETPUBLICKEY] = '"+ walletPublicKeyOwner + "'"
                    +"  ,[USER_SECRETPK] = '"+ payingPK + "'"
                    +"  ,[USER_PAYINGPK] = '"+ payingPK + "'"
                    +"  ,[NFT_SHARD_SERENITY] = '"+ solSereNftPK + "'"
                    +"  ,[NFT_SHARD_USER] = '"+ solUsrNftPK + "'"
                    +"  ,[NFT_SHARD_HEIR] = '"+ solHeirNftPKs + "'"
        ...
        public bool IfExistStrongboxByIdUser(string idUser)
        {
            bool exist = false;
            ...
            connexDB.Open();
            OleDbCommand commDB;
            commDB = new OleDbCommand("if exists ( SELECT distinct   STRONGBOXPERSONNAL.* "
                                      + " FROM            STRONGBOXPERSONNAL INNER JOIN"
                                      + " CUSTOMER ON STRONGBOXPERSONNAL.ID_CUSTOMER = CUSTOMER.ID_CUSTOMER INNER JOIN"
                                      + " _USER ON CUSTOMER.ID_USER = _USER.ID_USER where _user.ID_USER ='" + idUser + "' AND STRONGBOXPERSONNAL.ACTIVE=1 ) "
                            + " select 'True'  "
                            + " else "
